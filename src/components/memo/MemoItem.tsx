...
import { MemoData } from '../../app/actions/memo'
import moment from 'moment'
import { useAppSelector } from '../../app/hooks';
import { THEME_COLOR } from '../../App.theme';
import { FolderDetail } from '../../app/actions/folder';
...
const MemoItem: React.FC<MemoItemsProps> = ({ data: [ createAt, detail ] }) => {
  ...
  const isAllFolder = folder.activeFolder === '';
  const targetFolder:FolderDetail = folder.data[detail.included];

  return (
    <MemoItemContents $active={memo.activeMemo === createAt}>
      <MemoItemTitle $isPinned={detail.isPinned}>
        <span>{ detail.title || '새로운 메모' }</span>
      ...
      <p>
        <TimeStampEl>{ moment(detail.updateAt).fromNow() }</TimeStampEl>
        { 
        isAllFolder && 
          <FolderTagEl $color={THEME_COLOR[targetFolder.colorChip] ?? 'none'}>
...
// styled components
const MemoItemContents = styled.li<{ $active: boolean }>`
  display: block;
  width: 100%;
  height: auto;
  padding: 8px 12px;
...
const MemoItemTitle = styled.h3<{ $isPinned: boolean }>`
  display: block; 
  width: 100%; 
  height: auto;
  font-size: 15px;
