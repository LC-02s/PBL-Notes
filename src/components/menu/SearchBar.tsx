...
export default function SearchBar() {
  ...
  const handleSearchResultBlur = (index: number) => {
    if (index === searchResult.length - 1) setIsFouced(false);
  }
  ...
  return (
    <SearchContainer ref={searchArea} $isFocused={isFouced}>
      ...
      <input type='text' placeholder='검색어를 입력해주세요' ref={inputRef} value={inputValue} onChange={handleInputChange} onFocus={() => setIsFouced(true)} />
      <button type='button' disabled={inputValue ? false : true} onClick={handleInitSearchValueBtnClick}>
        ...
        searchResult.length > 0 ? 
          <ul>
            { searchResult.map(({ title, createAt }, idx) => (<li key={String(createAt)}>
              <button type='button' onClick={() => handleSearchResultClick(createAt)} onBlur={() => handleSearchResultBlur(idx)}>
                { title }
