...
import { changePinnedState } from '../../../app/actions/note';

export default function PinBtn() {

  const { tempData } = useAppSelector(({ note }) => note);
  const isDisabled = tempData === null || !tempData.modifiable;
  const isActive = (tempData?.isPinned ?? false) && (tempData?.modifiable ?? true);
  const dispatch = useAppDispatch();

  return (
    <MenuToggleBtn $active={isActive} disabled={isDisabled} title={isActive ? '고정 해제' : '메모 고정'} onClick={() => dispatch(changePinnedState(null))}>
      {/* outline */}
