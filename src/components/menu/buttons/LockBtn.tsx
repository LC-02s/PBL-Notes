...
import { toggleMemoLock } from '../../../app/actions/memo';

export default function LockBtn() {

  const memo = useAppSelector(({ memo }) => memo);
  const activeMemo = memo.activeMemo;
  const isDisabled = activeMemo === '';
  const isActive = !isDisabled && memo.data[activeMemo].isLocked;
  const dispatch = useAppDispatch();
  const handleBtnClick = () => { dispatch(toggleMemoLock(activeMemo)); }

  return (
    <MenuToggleBtn $active={isActive} disabled={isDisabled} title={isActive ? '잠금 해제' : '메모 잠금'} onClick={handleBtnClick}>
      {/* outline */}
