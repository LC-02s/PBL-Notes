...
type MemoItemsProps = { data: Note };

const NoteItem: React.FC<MemoItemsProps> = ({ data }) => {

  const { folderList } = useAppSelector(({ folder }) => folder);
  const { pathname } = useLocation();
  const color = pathname === '/' && (() => {
    const targetIndex = folderList.findIndex(({ name }) => name === data.included);
    return targetIndex >= 0 ? folderList[targetIndex].color : false;
  })();

  return (
    <NoteItemContents $active={false}>
      ...
      <p>
        <TimeStampEl>{ moment(data.updateAt).fromNow() }</TimeStampEl>
        { 
        pathname === '/' && 
