...
type MemoItemsProps = { data: Note, index: number, targetName: string };

const NoteItem: React.FC<MemoItemsProps> = ({ data, index, targetName }) => {

  const { folderList } = useAppSelector(({ folder }) => folder);
  const { activeNoteId } = useAppSelector(({ note }) => note);
  const targetFolderIndex = targetName === 'all' ? folderList.findIndex(({ name }) => name === data.included) : -1;
  const color = folderList[targetFolderIndex]?.color ?? 'none';
  const dispatch = useAppDispatch();

  return (
    <NoteItemContents $active={activeNoteId === data.createAt}>
      <button type='button' onClick={() => dispatch(changeActiveNoteId(data.createAt))}>
