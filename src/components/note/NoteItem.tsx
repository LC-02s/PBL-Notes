...
import DragImg from '../../asset/note-drag.png'
import { toggleIsDragging } from '../../app/actions/ui';
...
const NoteItem: React.FC<NoteItemsProps> = ({ data, targetPath, folderMap }) => {
  ...
  return (
    <NoteItemContents $active={activeNoteId === data.createAt}>
      <button type='button' draggable={true} disabled={activeNoteId === data.createAt} onClick={() => dispatch(changeActiveNoteId(data.createAt))} onDragStart={(e) => handleBtnDragStart(e, data.createAt)} onDragEnd={handleBtnDragEnd}>
